/**
 * @fileOverview Contains code related to the TPG FormValidator class
...
(function( $ ) {
	/* Do not modify these: */ 
	/**
	 * @property {number} allows creation of unique IDs if multiple FormValidator instances are created on a page
	 */
	var idCounter = 0; 
	...
	function FormValidator(formGroupElement, triggerElement, customRulesObject) {
		...
		/** Set of validator rules. Default rules must be defined in FormValidatorConfig.js that loads before this class, on TPG.defaultValidatorRules
		 ...
		 * NOTE: teams should add, remove or modify these default rules where applicable
		 * @type Object 
		 */
		var _validatorRules = TPG.defaultValidatorRules ? TPG.defaultValidatorRules : {};
		
		/** Object containing Localized strings, where each key is a locale code, and each value is a nested object containing the 
		 * individual string IDs and their localized translation. These values need to be defined in FormValidatorConfig.js as TPG.validatorStrings
		 ...
		 * Note: User can add more locales as needed. As the FormValidator class evolves, additional strings may be added to each locale.
		 * @type object
		 */
		var _strings = TPG.validatorLocalizedStrings ? TPG.validatorLocalizedStrings : {};
		
		/**
		 * How long to wait for asynchronous validation callbacks before they timeout. Must be modified in FormValidatorConfig.js as TPG.validatorAsyncTimeoutDuration
		 * @type number
		 */
		var _asyncTimeoutDuration = TPG.validatorAsyncTimeoutDuration !== undefined ? TPG.validatorAsyncTimeoutDuration : 3000; 
		
		/**
		 * HTML strings used to generate the elements of the form validation summary list and inline errors.
		 * May only be overridden in FormValidatorConfig.js if you know what you're doing, NOT here
		 * @type Object
		 */
		var _markupStrings = {
		...
		if (TPG.validatorHTMLStrings) {
			$.each(_markupStrings, function(key, value){
				if (TPG.validatorHTMLStrings[key]) {
					_markupStrings[key] = TPG.validatorHTMLStrings[key] 
				}
			}); 
		}
		...
		if (TPG.customLogicForElementToFollow && typeof TPG.customLogicForElementToFollow == "function") {
			_customLogicForElementToFollow = TPG.customLogicForElementToFollow;
		...
		function _performBucketCollection() {
			...
			if (errors.length) {
				$(".errorSummaryContainer", _$formGroupContainer).find("a").first().focus();
	...
	/**
	 * Make class publically reachable. Create a new instance like this: var myValidator = new TPG.FormValidator(container);
	 * NOTE: This global variable can be changed if necessary, it won't break the class
	 * 
	 */
	TPG.FormValidator = FormValidator; 
})( jQuery );